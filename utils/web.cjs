...
const subscriptionContext = '/subs-noauth/v1';
...
const trigger = function (apiKey, teamName,
    ...
    if (env == 'staging') {
        endpoint = 'https://stg-gateway.qyrus.com';
    }
    else if (env == 'qyrus') {
    ...
    function continueExecution() {
        ...
        function proceedWithStatusCheck(finalbody, teamId) {
            ...
            var scriptResultStatus = {
                host: hostName,
                port: port,
                path: '/checkScriptExecutionStatus',
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'scope': 'NODE_CLI'
...
function checkExecStatus(execStatus, triggerResponse, testSuite,
    ...
    execStatus.path = webContext + '/api/test-report-details?runId=' + runId;
    execStatus.headers = {
        'x-api-key': token,
        'Content-Type': 'application/json',
        'scope': 'NODE_CLI',
...
function checkScriptStatus(scriptResultStatus, runId, token, teamId) {
    ...
    scriptResultStatus.path = webContext + '/api/test-report-details?runId=' + runId;
    ...
    var reqPost = https.request(scriptResultStatus, function (res) {
        ...
        res.on('end', chunk => {
            if ((body === null) || (typeof body === 'undefined') || (body === "")) {
                ...
                try {
                    // console.log("Script Execution Status Response: " + body);
                    var parsedJson = JSON.parse(body);
                    if (parsedJson.scriptExecution) {
...
function checkFinalStatus(finalResult, triggerResponse, testSuite, emailId, runId, token, teamId) {
    ...
    finalResult.path = webContext + '/api/test-report-details?runId=' + runId;
    finalResult.headers = {
        'x-api-key': token,
        'Content-Type': 'application/json',
        'Team-Id': teamId,
...
function getReportUrl(host, port, path, token, finalStatus, testSuite, internalMappings, teamId, emailId) {
    ...
    const options = {
        host: host,
        port: port,
        path: webContext + '/api/get-reports-url?path=' + encodeURIComponent(path),
        method: 'GET',
        headers: {
            'x-api-key': token,
            'Content-Type': 'application/json',
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            const options = {
                host: hostName,
                port: port,
                path: webContext + `/api/getOrganizationByTeam?teamName=${encodeURIComponent(teamName)}`,
                method: 'GET',
                headers: {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const options = {
                host: hostName,
                port: port,
                path: webContext + `/api/projects-by-team-and-servicestore?teamId=${teamId}&serviceStoreId=${serviceStoreId}`,
                method: 'GET',
                headers: {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const options = {
                host: hostName,
                port: port,
                path: webContext + `/api/test-suites-for-test-lab?projectUUID=${projectUUID}`,
                method: 'GET',
                headers: {
                    'x-api-key': customAuth,
                    'scope': 'NODE_CLI',
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const options = {
                host: hostName,
                port: port,
                path: webContext + `/api/variables-environment?projectUUID=${projectUuid}`,
                method: 'GET',
                headers: {
                    'x-api-key': customAuth,
                    'scope': 'NODE_CLI',
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const options = {
                host: hostName,
                port: port,
                path: subscriptionContext + `/api/get-active-browser-subscriptions/${teamId}?login=${login}`,
                method: 'GET',
                headers: {
                    'x-api-key': authToken,
                    'scope': 'NODE_CLI',
        ...
        getActiveBrowserSubscriptions(gatewayUrl, userAuthToken, teamId, login).then(function (browserConfig) {
            ...
            try {
                ...
                const options = {
                    host: hostName,
                    port: port,
                    path: webContext + '/api/execute-test',
                    method: 'POST',
                    headers: {
